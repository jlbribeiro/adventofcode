...
func RemainingSteps(bestPaths map[rune]map[rune][]VisitPath, cache map[CacheKey]int, curPositions [NRobots]rune, allKeys KeySet, curKeyset KeySet) int {
	...
	// Usually, the CacheKey.AtKey field's value (curPositions) would have to be
	// sorted in order to ensure a consistent hash per state
	// (and maximize cache hits); however, the algorithm only needs so few
	// modifications from part 1 because each key is only reachable by 1 robot
	// (since the sections the robots are in are completely separate).
	// Therefore, sorting is unnecessary (as no permutations can occur) and,
	// as such, slower.

	cacheKey := CacheKey{AtKey: curPositions, RemainingKeys: remainingKeys}
	...
	for remainingKeyID := 0; keyIDToBitMask(remainingKeyID) <= int(allKeys); remainingKeyID++ {
		...
		for robotID, curKey := range curPositions {
			...
			// Copy previous positions, change chosen robot to remainingKey
			newPositions := curPositions
			newPositions[robotID] = remainingKey

			collectionSteps := minCostToRemainingKey + RemainingSteps(bestPaths, cache, newPositions, allKeys, curKeyset.WithKey(remainingKey))
...
func DiscoverAllPaths(tunnels [][]rune, keysWithCoords map[rune]Vec2, initialCoords [NRobots]Vec2) map[rune]map[rune][]VisitPath {
	bestPaths := make(map[rune]map[rune][]VisitPath)

	for robotID, robotCoords := range initialCoords {
		bestPaths[rune('0'+robotID)] = PathsToKeysFrom(tunnels, keysWithCoords, robotCoords)
	}
...
func ShortestPathToKeys(tunnels [][]rune) int {
	var initialCoords [NRobots]Vec2
	robotID := 0
	...
	for y, row := range tunnels {
		for x, cell := range row {
			if isKey(cell) {
				...
				initialCoords[robotID] = Vec2{Y: y, X: x}
				robotID++
				tunnels[y][x] = Space
				continue
			}
	...
	if robotID != len(initialCoords) {
		panic(fmt.Sprintf("input must have %d robots", NRobots))
	}
	...
	var curPositions [NRobots]rune
	for robotID := range initialCoords {
		curPositions[robotID] = rune('0' + robotID)
	}

	cache := make(map[CacheKey]int)
	return RemainingSteps(bestPaths, cache, curPositions, allKeys, KeySet(0))
}
