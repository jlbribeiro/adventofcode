...
func RunFromInput(intcodeProgram io.Reader, userInput int) []int {
	inputS, err := ioutil.ReadAll(intcodeProgram)
	if err != nil {
		panic(err)
	}

	memoryS := strings.Split(strings.TrimSpace(string(inputS)), ",")
	memory := make([]int, len(memoryS))
	for i, nS := range memoryS {
...
func Run(mem []int, userInput int) []int {
	var output []int
	ip := 0
