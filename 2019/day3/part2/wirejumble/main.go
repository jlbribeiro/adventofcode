	...
	for i, wire := range *wires {
		...
		for _, seg := range wire.Segments {
			...
			for j := 0; j < seg.Length; j++ {
				...
				if grid[origY+y][origX+x].WireID == 0 {
					grid[origY+y][origX+x].WireID = i + 1
					grid[origY+y][origX+x].Steps = steps
					continue
				}

				// Wire intersecting itself
				if grid[origY+y][origX+x].WireID == i+1 {
					continue
				}

				// Second wire intersecting first wire
				curInterSteps := grid[origY+y][origX+x].Steps + steps
				minInterSteps = min(minInterSteps, curInterSteps)
			}
		}
	}
